...
class HAKlyqaAccount(api.Klyqa_account):
    ...
    def __init__(self, udp, tcp, username="", password="", host="", hass=None):
        super().__init__(username, password, host)
        self.hass = hass
        self.udp = udp
        self.tcp = tcp

    async def send_to_bulbs(self, args, args_in, timeout_ms=5000):
        """_send_to_bulbs"""
        return await super()._send_to_bulbs(
            args, args_in, self.udp, self.tcp, timeout_ms=timeout_ms
        )
